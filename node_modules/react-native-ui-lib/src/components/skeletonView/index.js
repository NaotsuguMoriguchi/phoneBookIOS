...
class SkeletonView extends Component {
  static propTypes = {
    ...
    /**
       ...
       */
    template: _pt.oneOf(["listItem", "content"]),
  ...
  renderListItemLeftContent = () => {
    ...
    if (contentType) {
      ...
      const style = {
        marginRight: size === Size.LARGE ? 16 : 14
      };
  ...
  renderListItemContentStrips = () => {
    ...
    return <View width={'100%'} height={height} centerV style={!hideSeparator && Dividers.d10}>
  ...
  renderAdvanced = () => {
    ...
    return <View style={style} {...this.getAccessibilityProps('Loading content')}>
  ...
  render() {
    ...
    if (times) {
      return _.times(times, index => {
        ...
        return <SkeletonView {...this.props} key={key} testID={`${testID}-${index}`} renderContent={index === 0 ? renderContent : this.renderNothing} hideSeparator={hideSeparator || !showLastSeparator && index === times - 1} times={undefined} />;
      });
    } else {
