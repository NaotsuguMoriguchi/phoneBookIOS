...
})(PanViewDirections || (PanViewDirections = {}));
...
function getVelocityDirectionClamp(event, directions) {
  ...
  if (directions.includes(PanViewDirections.LEFT) && event.velocityX < 0 || directions.includes(PanViewDirections.RIGHT) && event.velocityX > 0) {
    x = event.velocityX;
  }

  if (directions.includes(PanViewDirections.UP) && event.velocityY < 0 || directions.includes(PanViewDirections.DOWN) && event.velocityY > 0) {
    y = event.velocityY;
  }
...
function checkThresholds(directions, velocity, threshold, options) {
  ...
  const xPassedThreshold = directions.includes(PanViewDirections.RIGHT) && options.currentTranslation.x > threshold.x || directions.includes(PanViewDirections.LEFT) && -options.currentTranslation.x > threshold.x;
  const yPassedThreshold = directions.includes(PanViewDirections.DOWN) && options.currentTranslation.y > threshold.y || directions.includes(PanViewDirections.UP) && -options.currentTranslation.y > threshold.y;
  return {
